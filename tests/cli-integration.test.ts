...
describe('CLI Integration', () => {
  ...
  describe('tag case transformations', () => {
    it('should support kebab-case transformation', () => {
      ...
      const subDir = join(tempDir, 'MyComponents')
      mkdirSync(subDir, { recursive: true })
      const subFile = join(subDir, 'UserProfile.md')
      writeFileSync(subFile, '# User Profile Component')

      const mainContent = dedent`
        @@MyComponents/UserProfile.md
      `
      writeFileSync(testFile, mainContent)

      const output = execSync(
        `node ${originalCwd}/dist/cli.js --tag-case kebab "${testFile}"`,
        { encoding: 'utf-8' },
      )
    ...
    it('should support SHOUT case transformation', () => {
      ...
      const output = execSync(
        `node ${originalCwd}/dist/cli.js --tag-case shout --no-root-tag "${testFile}"`,
        { encoding: 'utf-8' },
      )
      ...
      const subDir = join(tempDir, 'MySection')
      mkdirSync(subDir, { recursive: true })
      const subFile = join(subDir, 'content.md')
      writeFileSync(subFile, '# Content')

      const mainContent = dedent`
        @@MySection/content.md
      `
      writeFileSync(testFile, mainContent)

      const output2 = execSync(
        `node ${originalCwd}/dist/cli.js --tag-case shout "${testFile}"`,
        { encoding: 'utf-8' },
      )

      expect(output2).toBe(dedent`
        <DOCUMENT>
          <MYSECTION>
            # Content
          </MYSECTION>
    ...
    it('should support meme case transformation', () => {
      const subDir = join(tempDir, 'Documentation')
      mkdirSync(subDir, { recursive: true })
      const subFile = join(subDir, 'GettingStarted.md')
      writeFileSync(subFile, '# Getting Started Guide')
      ...
      const output = execSync(
        `node ${originalCwd}/dist/cli.js --tag-case meme "${testFile}"`,
        { encoding: 'utf-8' },
      )
    ...
    it('should default to pascal case', () => {
      const subDir = join(tempDir, 'myFolder')
      mkdirSync(subDir, { recursive: true })
    ...
    it('should work with other transformations', () => {
      ...
      const dir2 = join(tempDir, 'Utils')
      mkdirSync(dir1, { recursive: true })
      mkdirSync(dir2, { recursive: true })

      writeFileSync(join(dir1, 'Header.md'), '# Header Component')
      writeFileSync(join(dir2, 'Helpers.md'), '# Helper Functions')
      ...
      const output = execSync(
        `node ${originalCwd}/dist/cli.js --tag-case kebab --lift-all-tags-to-root "${testFile}"`,
        { encoding: 'utf-8' },
      )
