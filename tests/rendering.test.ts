...
describe('renderToXml - Tag Flattening', () => {
  it('flattens consecutive same-named tags by default', () => {
    const parsedInfo: ParsedFsInfo = {
      nodes: [
        {
          type: 'wrapper',
    ...
    const result = renderToXml(parsedInfo, { flatten: true })
  ...
  it('does not flatten when option is disabled', () => {
    const parsedInfo: ParsedFsInfo = {
      nodes: [
        {
          type: 'wrapper',
    ...
    const result = renderToXml(parsedInfo, { flatten: false })

    expect(result.match(/<command>/g)?.length).toBe(2)
    expect(result.match(/<\/command>/g)?.length).toBe(2)
  })
  ...
  it('flattens nested tags with same name', () => {
          ...
          children: [
            {
              type: 'command',
              commandName: 'fs-to-xml',
              input: 'fs-to-xml rules/test1.md',
              stdout: '',
              stderr: '',
              exitCode: 0,
              fs2xmlNonShebang: true,
          ...
          children: [
            {
              type: 'command',
              commandName: 'fs-to-xml',
              input: 'fs-to-xml rules/test2.md',
              stdout: '',
              stderr: '',
              exitCode: 0,
              fs2xmlNonShebang: true,
    ...
    // Should have one <command> tag containing both fs-to-xml commands
    expect(result.match(/<command>/g)?.length).toBe(1)
    expect(result.match(/<fs-to-xml>/g)?.length).toBe(2)
  })
})
